...
func _init(_name: String, _display_name: String, _description: String, _eval_randomness: float, _default_depth: int, _enemy_credits: int) -> void:
	self.name = _name
	self.display_name = _display_name
	self.description = _description
	self.eval_randomness = _eval_randomness
	self.default_depth = _default_depth
	self.enemy_credits = _enemy_credits
